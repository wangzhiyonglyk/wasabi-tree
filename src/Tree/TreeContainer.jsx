...
import formatterData from "../libs/formatterData.js";
...
function TreeContainer(props, ref) {
    ...
    const loadSuccess = useCallback((res) => {
        ...
        row = JSON.parse(row);
        //格式化数据
        let asyncChildrenData = formatterData("tree", "", realData, props.idField || "id", props.textField || "text", props.parentField || "pId", props.simpleData);
        ...
        if (props.asyncAble && (!row.children || row.children.length == 0)) {//没有数据
            ...
            if (props.onAsync && typeof props.onAsync === "function") {//自行处理
                ...
                if (asyncChildrenData && asyncChildrenData instanceof Array && asyncChildrenData.length > 0) {
                    //格式化数据
                    asyncChildrenData = formatterData("tree", "", asyncChildrenData, props.idField || "id", props.textField || "text", props.parentField || "pId", props.simpleData);
                    dispatch({ type: "append", payload: { children: asyncChildrenData, row: row } })
    ...
    */
    ...
    useEffect(() => {
        if (props.url) {
            ...
            let startIndex = Math.floor(scrollTop / config.rowDefaultHeight);
            let endIndex = startIndex + config.bufferScale * visibleDataCount;
            let data = formatterData("tree", null, props.data, props.idField, props.textField, props.parentField, props.simpleData);
